...
AS	
  SELECT row_number() OVER (ORDER BY p.pollutant_cas_id) AS id,
   ...
   FROM emission e
     ...
     LEFT JOIN ( SELECT sum(e_0.calculated_emissions_tons * rpa_0.percent / 100::numeric) AS calculated_emissions_tons, sum(e_0.total_emissions * rpa_0.percent / 100::numeric) AS total_emissions,
            er_0.year,
     ...
     LEFT JOIN ( SELECT sum(e_1.calculated_emissions_tons * rpa_1.percent / 100::numeric) AS calculated_emissions_tons, sum(e_1.total_emissions * rpa_1.percent / 100::numeric) AS total_emissions,
            er_1.year,
     ...
     LEFT JOIN ( SELECT sum(e_2.calculated_emissions_tons * rpa_2.percent / 100::numeric) AS calculated_emissions_tons, sum(e_2.total_emissions * rpa_2.percent / 100::numeric) AS total_emissions,
            er_2.year,
  ...
  GROUP BY er.year, fs.id, p.pollutant_code, p.pollutant_cas_id, p.pollutant_name, e.emissions_uom_code, p.pollutant_type, 
  ...
  previous_year_total.calculated_emissions_tons, previous_year_total.total_emissions, previous_year_total.year;