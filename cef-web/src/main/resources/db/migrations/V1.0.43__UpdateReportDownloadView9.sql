...
SELECT row_number() OVER (ORDER BY p.pollutant_cas_id) AS id,
    ...
    replace(replace(replace(e.emissions_factor_text::text, ','::text, ''::text), Chr(13)::text, ' '::text), Chr(10)::text, ' '::text) AS emissions_factor_text,
    replace(replace(replace(e.comments::text, ','::text, ''::text), Chr(13)::text, ' '::text), Chr(10)::text, ' '::text) AS emissions_comment,
    replace(replace(replace(e.calculation_comment::text, ','::text, ''::text), Chr(13)::text, ' '::text), Chr(10)::text, ' '::text) AS calculation_comment,
    e.total_emissions
   FROM emission e
