 ...
 SELECT row_number() OVER (ORDER BY p.pollutant_cas_id) AS id,
    ...
    previous_year_total.calculated_emissions_tons AS previous_year_total,
   ...
   FROM emission e
     ...
     LEFT JOIN ( SELECT sum(e_0.calculated_emissions_tons) AS calculated_emissions_tons,
            er_0.year,
          ...
          WHERE rptc_0.description::text = 'Fugitive'::text and ep_0.status_code = 'OP'
     ...
     LEFT JOIN ( SELECT sum(e_1.calculated_emissions_tons) AS calculated_emissions_tons,
          ...
          WHERE rptc_1.description::text <> 'Fugitive'::text and ep_1.status_code = 'OP'
     ...
     LEFT JOIN ( SELECT sum(e_2.calculated_emissions_tons) AS calculated_emissions_tons,
            er_2.year,
          ...
          GROUP BY er_2.year, p_2.pollutant_cas_id, p_2.pollutant_code, er_2.id) previous_year_total ON 
            previous_year_total.year = (select max(year) from emissions_report where frs_facility_id = er.frs_facility_id and year < er.year)
            AND previous_year_total.pollutant_code::text = p.pollutant_code::text AND previous_year_total.facility_id = er.frs_facility_id
  where ep.status_code = 'OP'
  GROUP BY er.year, fs.id, p.pollutant_cas_id, p.pollutant_name, 'tons'::text, p.pollutant_type, (COALESCE(fugitive_amount.calculated_emissions_tons, 0::numeric)), (COALESCE(stack_amount.calculated_emissions_tons, 0::numeric)), previous_year_total.calculated_emissions_tons, previous_year_total.year;