 ...
 SELECT row_number() OVER (ORDER BY p.pollutant_cas_id) AS id,
    ...
    p.pollutant_name,
    p.pollutant_type,
    COALESCE(rtrim(fugitive_amount.calculated_emissions_tons::text, '00'::text)::numeric, 0::numeric) AS fugitive_total,
    COALESCE(rtrim(stack_amount.calculated_emissions_tons::text, '00'::text)::numeric, 0::numeric) AS stack_total,
    'tons'::text AS uom,
    ...
    rtrim(previous_year_total.calculated_emissions_tons::text, '00'::text)::numeric AS previous_year_total,
