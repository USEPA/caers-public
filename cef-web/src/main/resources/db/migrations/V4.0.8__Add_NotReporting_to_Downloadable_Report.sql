...
-- Recreate the view with the new column `e.not_reporting`
CREATE OR REPLACE VIEW vw_report_download AS
SELECT
    row_number() OVER (ORDER BY p.pollutant_cas_id) AS id,
    er.id AS report_id,
    ...
    replace(replace(replace(e.emissions_factor_text::text, ','::text, ''::text), chr(13), ' '), chr(10), ' ') AS emissions_factor_text,
    replace(replace(replace(e.comments::text, ','::text, ''::text), chr(13), ' '), chr(10), ' ') AS emissions_comment,
    replace(replace(replace(e.calculation_comment::text, ','::text, ''::text), chr(13), ' '), chr(10), ' ') AS calculation_comment,
    e.total_emissions,
    rtrim((e.total_emissions * rpa.percent / 100::numeric)::text, '00')::numeric AS apportioned_emissions,
    ...
    e.not_reporting -- ✅ New column added here
FROM emission e
