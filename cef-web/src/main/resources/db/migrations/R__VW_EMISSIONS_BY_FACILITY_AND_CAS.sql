...
CREATE OR REPLACE VIEW vw_emissions_by_facility_and_cas
 ...
 SELECT concat(e.id, rpa.id) AS id,
  ...
  WHERE (er.status::text = ANY (ARRAY['SUBMITTED'::character varying::text, 'APPROVED'::character varying::text])) AND ep.status_code::text = 'OP'::text AND er.is_deleted = false;
