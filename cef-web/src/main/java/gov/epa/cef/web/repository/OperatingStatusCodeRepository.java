...
public interface OperatingStatusCodeRepository extends CrudRepository<OperatingStatusCode, String> {
    ...
    @QueryHints({
        @QueryHint(name = "org.hibernate.cacheable", value = "true")})
    @Query("select osc from OperatingStatusCode osc where osc.subFacilityStatus = true and osc.code = :code")
    Optional<OperatingStatusCode> findSubFacilityStatusEntityByCode(String code);
}
